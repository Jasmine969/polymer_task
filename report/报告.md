...
为了测试单链拓扑能否被 GROMACS 正确读取，先手写最小主拓扑文件 `single_chain/topol_raw.top`，包含刚生成的 `CLS_single_raw.itp`：
...
其中 `[ defaults ]` 和 `[ atomtypes ]` 是必要的，可以从原始 `topology/CLS_10chains.itp` 中直接拷贝。单链 ITP 中的 `[ atoms ]` 使用了 `hc`、`c3`、`h2`、`h1`、`oh`、`ho`、`os` 等 atom types，但 ITP 本身没有定义这些 atom types。如果主拓扑中缺少 `[ atomtypes ]`，`grompp` 会无法识别这些原子类型。
...
在服务器 GROMACS 2022.2-dev 环境下，该命令成功生成 `.tpr`。关键输出包括：
...
这说明 `atomtypes`、`defaults`、moleculetype 名称、原子数以及 bonded interactions 在 GROMACS 预处理层面可以被正确读取。输出中的 PME mesh load 是性能提示，不是拓扑错误。也就是说，该拓扑虽然能通过 `grompp`，但**仍需要进一步检查化学键连是否合理**。

## 2.4 可视化与价态检查

虽然 `grompp` 可以生成 `.tpr`，但这只能说明拓扑在 GROMACS 语法和参数层面可被解释，并不保证化学键连完全合理。因此进一步将单链体系导出为带键连信息的 PDB：

```bash
python scripts/task2/export_single_chain_pdb.py
```

该脚本使用 ParmEd 从主拓扑和 `single_chain/CLS_single.gro` 读取拓扑与坐标，并写出带 `CONECT` 记录的 PDB，便于在 OVITO 中查看拓扑定义的键连关系。此时主要检查未经修正的 `CLS_single_raw.pdb`。

可视化后发现，`H1` 与 `H4` 两个氢原子之间存在一条直接 H-H 键，使两个氢原子都变成二配位。进一步运行价态检查脚本：
...
```text
atom 1 = H1，已与 atom 2 = C1 成键
atom 9 = H4，已与 atom 5 = C4 成键
但拓扑中又额外定义了 atom 1 与 atom 9 的 H-H 键
```
...
修正由脚本 `scripts/task2/remove_hydrogen_hydrogen_bonds.py` 完成：
...
随后手写 `single_chain/topol_corrected.top`，其结构与 `topol_raw.top` 相同，只是将 `#include` 的文件改为修正后的 ITP：
...
随后再次运行同一脚本，同时刷新 raw 和 corrected 两个 PDB：

```bash
python scripts/task2/export_single_chain_pdb.py
```

其中修正后的 `CLS_single_corrected.pdb` 中，对应键连应为：
