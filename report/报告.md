...
原有的`solvent/water.itp`是不可用的。首先需要把`[ virtualsite3 ]`改为`[virtual_sites3]`，后者才是GROMACS可识别的directive。
...
ERROR 1 [file water_virtual_sites3_test.itp, line 29]:
  Duplicate atom index (3) in virtual_sites3
...
Source file: src/gromacs/gmxpreprocess/vsite_parm.cpp (line 994)

Fatal error:
There were 1 virtual sites involved in constraints
```

## 3.1 任务三输入文件整理说明

任务三相关输入并不全部由 `scripts/task3/prepare_task3_inputs.py` 生成。下表按输入文件汇总其数据来源和生成方法。

| 输入文件 | 作用 | 数据来源 | 生成方法 |
| --- | --- | --- | --- |
| `solvent/ethanol.itp` | 定义乙醇 `ETH` 分子的 GROMACS 拓扑，包括原子、电荷、键、角和二面角。 | ACPYPE 输出 `solvent/smiles_molecule.acpype/smiles_molecule_GMX.itp` | 由 `write_clean_ethanol_itp` 生成；从 `[ moleculetype ]` 开始保留，略去 ACPYPE 原文件中的 `[ atomtypes ]`；把 molecule name 从 `smiles_molecule` 改为 `ETH`；把 residue name 从 `UNL` 改为 `ETH`。 |
| `solvent/ethanol.gro` | 保存单个乙醇分子的整理后坐标，便于检查和作为 PDB 导出的中间坐标。 | ACPYPE 输出 `solvent/smiles_molecule.acpype/smiles_molecule_GMX.gro` | 由 `write_clean_ethanol_coordinates` 生成；用 ParmEd 读取乙醇坐标；把 residue name 改为 `ETH`；保留原 GRO 盒子尺寸。 |
| `solvent/ethanol.pdb` | 作为 PackMol 放置乙醇分子的结构模板。 | 整理后的 `solvent/ethanol.gro` | 由 `write_clean_ethanol_coordinates` 通过 `pdb_io.export_pdb` 导出；作为 PackMol 乙醇模板，不添加 `CONECT`。 |
| `single_chain/CLS_single_packmol.pdb` | 作为 PackMol 放置单条聚合物链的结构模板。 | `single_chain/CLS_single.gro` | 由 `write_origin_centered_cls_pdb` 生成；读取单链坐标，把几何中心平移到原点附近；把 residue name 统一为 `CLS`；导出 PackMol 用 PDB。 |
| `solvent/water_corrected.itp` | 定义水分子 `SOL` 的 GROMACS 拓扑，供两套溶剂体系 include。 | `solvent/water.itp` 中的 SPC/E 分子定义，以及对 GROMACS 2022.2 的 `grompp` 验证结果 | 文件较简单，由用户手动维护；不由 `prepare_task3_inputs.py` 写出。该文件不包含 `[ atomtypes ]`，只保留可通过当前 GROMACS 的水分子定义。 |
| `solvent/water.pdb` | 作为 PackMol 放置水分子的结构模板。 | `solvent/water.itp` 中 `[ settles ]` 的 `doh = 0.1000 nm` 和 `dhh = 0.1630 nm` | 由 `write_water_pdb_template` 计算生成；将 SETTLE 几何从 nm 换算到 Angstrom，按标准 PDB 坐标列 `8.3f` 写出 PackMol 用水分子模板。 |
| `solvent/forcefield_gaff_spce.itp` | 提供全体系共享的 `[ defaults ]` 和 atomtypes，避免分子 ITP include 时出现 directive 顺序错误。 | `single_chain/topol_corrected.top`、ACPYPE 原始乙醇 ITP `solvent/smiles_molecule.acpype/smiles_molecule_GMX.itp`、`solvent/water.itp` | 由 `write_forcefield_file` 自动解析生成；读取 `topol_corrected.top` 的 `[ defaults ]`，合并三个来源中的 `[ atomtypes ]`，按 atomtype 名称去重，并检查重复 atomtype 的参数是否一致。 |
| `systems/water/topol.top` | 纯水体系主拓扑，供 `grompp` 生成 EM/NVT/NPT/production 的 `.tpr`。 | `forcefield_gaff_spce.itp`、`CLS_single.itp`、`water_corrected.itp` 和体系分子数组成 | 用户手动编写；先 include 力场头文件，再 include 分子 ITP；`[ molecules ]` 写 `CLS 1` 和 `SOL 1500`。 |
| `systems/water_ethanol/topol.top` | 水/乙醇混合溶剂体系主拓扑，供 `grompp` 生成 EM/NVT/NPT/production 的 `.tpr`。 | `forcefield_gaff_spce.itp`、`CLS_single.itp`、`water_corrected.itp`、`ethanol.itp` 和体系分子数组成 | 用户手动编写；先 include 力场头文件，再 include 分子 ITP；`[ molecules ]` 写 `CLS 1`、`SOL 1050`、`ETH 450`。 |
| `systems/water/packmol.inp` | PackMol 输入文件，用于生成纯水体系初始 PDB 盒子。 | `CLS_single_packmol.pdb`、`water.pdb`、体系分子数和盒子尺寸 | 用户手动编写；写入聚合物和水模板的数量、`inside box` 范围，以及后续 GRO 转换用盒子长度注释。 |
| `systems/water_ethanol/packmol.inp` | PackMol 输入文件，用于生成水/乙醇体系初始 PDB 盒子。 | `CLS_single_packmol.pdb`、`water.pdb`、`ethanol.pdb`、体系分子数和盒子尺寸 | 用户手动编写；写入聚合物、水、乙醇模板的数量、`inside box` 范围，以及后续 GRO 转换用盒子长度注释。 |
| `systems/water/initial.pdb`、`systems/water_ethanol/initial.pdb` | PackMol 输出的初始混合结构，供可视化检查和后续 GRO 转换。 | 对应体系目录下的 `packmol.inp` | 在 WSL2 的 `condamd` 环境中运行 `packmol < packmol.inp` 生成。PDB 坐标单位为 Angstrom。 |
| `systems/water/initial.gro`、`systems/water_ethanol/initial.gro` | GROMACS 初始坐标文件，供 `grompp` 生成后续 EM 输入。 | 对应体系的 `initial.pdb`、单链/乙醇 GRO 模板、手动指定的分子数和盒子长度 | 由 `scripts/task3/packmol_pdb_to_gro.py` 生成；脚本用 ParmEd 读取 PackMol PDB 坐标，用 `scripts.utils.gmx_io.load_gro` 读取 CLS 和 ETH 的 GRO 模板，再按拓扑中的分子顺序写出 GRO。 |
| `mdp/task3_nvt.mdp`、`mdp/task3_npt.mdp`、`mdp/task3_prod.mdp` | 分别作为 NVT、NPT 和 production 阶段的 GROMACS 参数文件。 | `mdp/nvt.mdp`、`mdp/npt.mdp`、`mdp/nvt_prod.mdp` | 由 `write_task3_mdp_files` 生成；把温控组改为 `System`；为 NVT 添加生成初速度设置；为 NPT 和 production 添加从 checkpoint 接续的设置。 |

任务三的整体流程如下：

```mermaid
flowchart TD
    A[ACPYPE 生成乙醇原始文件<br/>smiles_molecule_GMX.itp / .gro] --> B[整理乙醇文件<br/>ethanol.itp / ethanol.gro / ethanol.pdb]
    C[单链坐标<br/>CLS_single.gro] --> D[生成 PackMol 聚合物模板<br/>CLS_single_packmol.pdb]
    E[SPC/E 水参数<br/>water.itp] --> F[手动维护修正水拓扑<br/>water_corrected.itp]
    E --> G[计算水分子模板<br/>water.pdb]
    H[聚合物/乙醇/水 atomtypes] --> I[统一力场头文件<br/>forcefield_gaff_spce.itp]

    B --> J[手动编写两套体系 topol.top]
    F --> J
    I --> J
    D --> K[手动编写 PackMol 输入<br/>packmol.inp]
    G --> K
    B --> K

    K --> L[PackMol 打包<br/>initial.pdb]
    L --> M[PDB 转 GRO<br/>initial.gro]
    J --> N[gmx_mpi grompp<br/>EM 输入 em.tpr]
    M --> N
    N --> O[gmx_mpi mdrun<br/>能量最小化]
    O --> P[NVT 平衡]
    P --> Q[NPT 平衡]
    Q --> R[Production 运行]

    subgraph S[两套体系分别执行]
        W[纯水体系<br/>CLS 1 + SOL 1500]
        X[水/乙醇体系<br/>CLS 1 + SOL 1050 + ETH 450]
    end
...
1. 已整理乙醇输入文件：`ethanol.itp`、`ethanol.gro` 和 `ethanol.pdb`。
2. 已准备水分子相关输入：手动维护 `water_corrected.itp`，并由脚本根据 `water.itp` 中的 SETTLE 几何计算生成 `water.pdb`。
3. 已由脚本合并聚合物、乙醇和水的 `[ atomtypes ]`，生成 `forcefield_gaff_spce.itp`。
4. 已手动编写两套体系的 `topol.top` 与 `packmol.inp`。
5. 已运行 PackMol 生成两套体系的 `initial.pdb`，并通过 `packmol_pdb_to_gro.py` 转换为 `initial.gro`。
6. 已用 `gmx_mpi grompp -f ../../mdp/em.mdp -c initial.gro -p topol.top -o em.tpr` 分别检查纯水体系和水/乙醇体系，二者均可生成 EM 阶段的 `em.tpr`。

PackMol 输入中的盒子尺寸目前按保守初始构型设置：纯水体系使用约 `50 Å = 5.0 nm` 的立方盒子，水/乙醇体系使用约 `58 Å = 5.8 nm` 的立方盒子。这里的盒子尺寸不是最终平衡密度对应的盒子尺寸，而是为了降低 PackMol 打包难度和初始严重重叠风险。后续 NPT 平衡会根据压力耦合调整盒子体积和体系密度。水/乙醇体系没有沿用 `5.0 nm`，主要是因为乙醇分子明显大于水分子；虽然两套体系溶剂分子数同为 1500 个量级，但混合体系总原子数更多，初始放置时更容易拥挤，因此先给更大的初始盒子。若需要增强两套体系的可比性，也可以后续尝试把混合体系改为 `5.0 nm` 起始盒子，再通过 PackMol 收敛情况、可视化、EM 最大力和 NPT 密度稳定性判断是否可用。
